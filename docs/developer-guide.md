...
**Di belakang Cloudflare Tunnel (production `school`):** `docker-compose.yml` sets `AGENTPAIR_TRUST_PROXY=1`. cloudflared connects from loopback/Docker bridge and forwards `X-Forwarded-For` / `CF-Connecting-IP` with the real client address.
