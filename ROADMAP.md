...
- [x] **M3.4a** `inbox_wait` (SPEC A.2): blocking MCP wait primitive wrapping `handleInbox`; keeps the agentic loop alive during live sessions without manual poll loops. **Prerequisite for M3.4 dogfood** — requires an ad-hoc **0.x patch** publish of `agentpair` on npm before dry-run (package currently at 0.1.11); this is **not** M4.4's `agentpair@1.0.0` release, which lands later in Milestone 4. `label:dx` `size:M`
