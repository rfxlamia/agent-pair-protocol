...
**Done (v0):** crypto suite, SPAKE2 pairing + wordlist, relay (default-deny inbox, challenge-response pull, content-addressed artifacts + quota + rate limit), MCP server (consolidated tools, via_human gates, session state machine with N1/N3, Zod payload validation, disk persistence), runners (spectral, codegen-compile, payload-size), e2e happy path, docker deploy. Published: `agentpair` 0.1.11, `@agentpair/protocol` 0.1.2.
